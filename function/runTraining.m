...
param.defaultWin = imdata.respRange;
logging.info("Default gating window = %0.1f", param.defaultWin)
...
%%
if isfield(ptdata,'net')
    layers = ptdata.net.Layers;
    nSecs = param.nSecs;
    ...
    net = trainNetwork(InData',OtData,layers,options);

    % Training results
    yData = predict(net,InData,'MiniBatchSize',1);
    eData = OtData - yData;

    err_data = 10 * log10(sum(eData.^2,'all') / sum(OtData.^2,'all'));
    logging.info('Network %2i, Train Err: %.2f.', nSecs, err_data)

    yData = cat(1, nan(nBeats,3), yData);
    OtData = imdata.shiftvec;
    OtData(1:nBeats,:) = nan;
    param.figName = genPlots(OtData, yData, param);
    param.yData = yData;
else
    switch netstruct
        case 'LSTM'
    ...
    parfor nSecs=1:2
        ...
        err_data = 10 * log10(sum(eTrain.^2,'all') / sum(OtTrain.^2,'all'));
        err_cv(nSecs) = 10 * log10(sum(eCV.^2,'all') / sum(OtCV.^2,'all'));

        logging.info('Network %2i, Train Err: %.2f, CV Err: %.2f.', nSecs, err_data, err_cv(nSecs))
