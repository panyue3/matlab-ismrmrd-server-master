...
% BW = im2bw(IMG/src_max,level*0.3);
% BW = imfill(BW,'holes');
% BW = ones(M,N,K);
BW = ones(M,N,K);
level = graythresh(IMG/src_max);
for ii=1:K
    BW(:,:,ii) = im2bw(IMG(:,:,ii)/src_max,level*0.3);
    BW(:,:,ii) = imfill(BW(:,:,ii),'holes');
end
%降采样4倍不错，越小校正越强
%作者使用的好像是3
resample = 4;
...
% K进行降采样时,C++代码中可能会比较麻烦,暂时先不做
resampleK = 1;
img = IMG(1:resample:M, 1:resample:N, 1:resampleK:K);
v_uc = log(img+eps);
[m,n,k] = size(v_uc);

bw = BW(1:resample:M, 1:resample:N, 1:resampleK:K);
BW = reshape(BW, 1, M*N*K);
bw = reshape(bw, 1, m*n*k);
...
fs_temp = zeros(m,n,K);
for ii=1:n
    fs_temp(:,ii,:) = reshape(imresize(squeeze(fs_last(:,ii,:)),[m,K]),m,1,K);
end
fs(:,:,1:K) = imresize(fs_temp(:,:,1:K), [M, N]);
fs = exp(fs);
vfs = fs(find(BW));
minfs = min(vfs);
fs = fs ./ minfs;   %这个处理需要再确认

fs  = reshape(fs, 1, M*N*K);
img = reshape(IMG, 1, M*N*K);
for ii=1:M*N*K
    if BW(ii)
        img(ii) = IMG(ii)./fs(ii);
    elseif fs(ii)>1
...
imtool(squeeze(IMG(:,:,round(K/2)+3)),[]);
imtool(squeeze(img(:,:,round(K/2)+3)),[]);
imtool(squeeze(fs(:,:,round(K/2)+3)),[]);
imtool(squeeze(BW(:,:,round(K/2))),[]);
toc;
