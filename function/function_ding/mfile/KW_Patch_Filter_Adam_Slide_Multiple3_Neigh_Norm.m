...
% [a_f, Noise_map] = KW_Patch_Filter_Adam_Slide_Multiple3_Neigh_Norm(a_0, varargin)
% comparing to KW_Patch_Filter_Adam_Slide, this function register patches
% mutiple times = 3 and filter it multiple times
% Neighbour shrinkage method
% Add noise normalization method

function [a_f, Noise_map, var_0] = KW_Patch_Filter_Adam_Slide_Multiple3_Neigh_Norm(a_0, varargin)

a_0 = a_0 + (a_0==0).*rand(size(a_0))*0.1;

[Noise_Map, Noise_Corr] = Stack_Noise_Map(a_0); % Ding 20240102
a_0 = a_0./Noise_Corr;
...
N_fr = T_win;
N_reg = 3; % 20231229 number of registration
N_sw = Tot_step_x*Tot_step_y*N_reg; % N_sw = Tot_step_x*Tot_step_y; 20231228
...
for k= 1:T_step:s_0(3)
    ...
    data_temp = zeros(Win_L, Win_L, T_win,  N_reg, Tot_step_x, Tot_step_y);
    filt_temp = zeros(Win_L, Win_L, T_win, N_sw);
    bp_x = zeros(Win_L, N_fr, N_reg, Tot_step_x, Tot_step_y);
    bp_y = zeros(Win_L, N_fr, N_reg, Tot_step_x, Tot_step_y);
    var_0 = zeros(1, N_sw);    
    t_filtered = zeros(s_0(1), s_0(2), T_win); % 20231228
    t_noise = zeros(s_0(1), s_0(2), T_win); % 20240102
    ...
    parfor j= 1:Tot_step_y%1:step_x:s_0(1)
        ...
        for i=1:Tot_step_x%1:step_y:s_0(2)             
            ...
            [data_temp(:,:,:,:,i,j), bp_x(:,:,:,i,j), bp_y(:,:,:,i,j)] = Multiple3_Alignment(Recon_1, x_ind, y_ind) ; % 20231228  
            %toc             
        end
    end
    data_temp = reshape(data_temp, [Win_L, Win_L, T_win, N_sw]);
    option_f.noise_var = 1;
    parfor i=1:N_sw
        %filt_temp(:,:,:,i) = KW_Filter(data_temp(:,:,:,i)); KW_Filter_Adam
        [filt_temp(:,:,:,i), var_0(i)] = KW_Filter_Adam_Neigh(data_temp(:,:,:,i), option_f); 
        %[filt_temp(:,:,:,i), var_0(i)] = KW_Filter_Adam_Neigh(data_temp(:,:,:,i)); 
    end
    filt_temp = reshape(filt_temp, [Win_L, Win_L, T_win, N_reg, Tot_step_x, Tot_step_y]);
    var_0 = reshape(var_0, [N_reg, Tot_step_x, Tot_step_y]);
    for i=1:Tot_step_x
        for j=1:Tot_step_y
            for k = 1:N_fr
                for l = 1:N_reg
                    ...
                    t_noise(x_ind, y_ind, k) = t_noise(x_ind, y_ind, k) + var_0(l, i, j);
    ...
    Noise_map(:,:,t_ind) = Noise_map(:,:,t_ind) + t_noise./a_weight;
