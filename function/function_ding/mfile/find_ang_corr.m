...
function [tt_out,ang_tt_out,qp_out,ang_qp_out,sl_out,ang_sl_out, hex_out,ang_hex_out] = find_ang_corr(a2,a2f,p2,p2f,a4,a4f,a5,a5f,p5,p5f,k0)
...
for A4=a4:100:a4f
    ...
    for A5_0 = a5:10:a5f
        ...
        for i=1:i00(3)
            ...
            ang2(i) = 49 + ang0_2 ; pause(0.01)
        end
       % hex,
 	    max_tt = max(tt(:)); 			N = find(tt == max(tt(:))) ;
	    max_sl = max(sl_1(:));			N_sl = find(sl_1 == max(sl_1(:))) ;
        max_qp = max(qp(:));			N_qp = find(qp == max(qp(:))) ;
        max_hex = max(hex(:));          N_hex = find(hex == max(hex(:)));

        tt_out(n4,n5) = max_tt(1);      ang_tt_out(n4,n5) = ang1(N(1)) ;
	    sl_out(n4,n5) = max_sl(1);		ang_sl_out(n4,n5) = ang_sl(N_sl(1)) ;
	    qp_out(n4,n5) = max_qp(1);		ang_qp_out(n4,n5) = ang_qp(N_qp(1)) ;
