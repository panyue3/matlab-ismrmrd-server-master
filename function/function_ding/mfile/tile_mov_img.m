% tile he last 10 images of the 20 images in the mov file and the No.11 image is the add up the total 10 
%
%
%
function tile_mov_img(fname)
    ...
    a0(1:160,1:120,i0) = a00;
    while prod(size(a00))==19200,
    a00=fread(fid,[160,120],'uchar');
    if prod(size(a00))==19200,i00=i00 + 1;a0(:,:,i0)=a00; end;
    end
    i00, 
    fclose(fid)        
    ...
    a00=sum(a,3);
    a00 = abs(fftshift(fft2(lpfilter(a00,0.08))));
    for i0 = 1:i00-1,
            a(:,:,i0) = a(:,:,i0)./ a00 ;
            afft0( 1:128, 1:128, i0  ) = abs( fftshift( fft2( a(:,:,i0 ) ) ) );  
        end
    a0 = a ;
    afft = sum(afft0,3); 
    afft(64:66,64:66,:)=0;
    figure(1), imagesc(afft);
    for i = 11 : 22,
