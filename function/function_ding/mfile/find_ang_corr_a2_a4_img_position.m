...
function [tt_out,ang_tt_out,qp_out,ang_qp_out,sl_out,ang_sl_out, hex_out,ang_hex_out,max_peak_tt,max_peak_sl,max_peak_qp,max_peak_hex,qp] = find_ang_corr_a2_a4_img_position(a2,a2f,p2,p2f,a4,a4f,a5,a5f,p5,p5f,k0)
...
for A4=a4:100:a4f
    ...
    for A5_0 = a5:10:a5f
        ...
        %sl_1,
 	    max_tt = find_max_average_ang(tt(:)); 			N = find(tt == max_tt(1)) ;
	    max_sl = find_max_average_ang(sl_1(:));			N_sl = find(sl_1 == max_sl) ;
        max_qp = find_max_average_ang(qp(:));			    N_qp = find(qp == max(qp(:)));% plot(qp,'*-'), %pause
        max_hex = find_max_average_ang(hex(:));           N_hex = find(hex == max_hex(1));

        tt_out(a2,n4) = max_tt(1);      ang_tt_out(a2,n4) = ang1(N(1)) ;        max_peak_tt(a2,n4) = N;
	    sl_out(a2,n4) = max_sl(1);		ang_sl_out(a2,n4) = ang_sl(N_sl(1)) ;   max_peak_sl(a2,n4) = N_sl;
	    qp_out(a2,n4) = max_qp(1);		ang_qp_out(a2,n4) = ang_qp(N_qp(1)) ;   max_peak_qp(a2,n4) = N_qp;
        hex_out(a2,n4) = max_hex(1);    ang_hex_out(a2,n4) = ang2(N_hex(1));    max_peak_hex(a2,n4) = N_hex;
        %hex,ang2
        pause(0.1)
    end
end ;   end ;   end ;   end
