...
while(button == 1)
    x=round(x); y=round(y);
    %disp(['Remove Region, x = ', num2str(x), ', y = ', num2str(y)])
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% disp('LN 385, ATTN: Shrink to Line Button Pressed')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% disp('LN 402, ATTN: Spur Image Button Pressed')
while(true), % disp('LN 403, ATTN: Ture')
    if(max_two_neigh(handles.binary_image)), % disp('LN 404, ATTN: IF')
        break;
    else, % disp('LN 406, ATTN: ELSE'), figure(2), imagesc(handles.binary_image), title('Before Spur'),
        handles.binary_image = bwmorph(handles.binary_image,'spur');
        guidata(hObject,handles);
        axes(handles.mainFigure);
        imshow(handles.binary_image); % figure(3), imagesc(handles.binary_image), title('After Spur'), pause(100)
    end
end
% disp('LN 413, Spur Image')
% save the center line
handles.aorta_central_line = handles.binary_image;
% disp('LN 416, Spur Image')
...
disp(['Pulse wave velocity = ' num2str(p(1)) ' cm/s.   R=' num2str(R)])
% --------------------------------------------------------------------
