...
function [a_f, Noise_map] = KW_Patch_Filter_Adam_Slide_20231213(a_0, varargin)
...
for k= 1:T_step:s_0(3)
    ...
    data_temp = zeros(Win_L, Win_L, T_win, Tot_step_x, Tot_step_y);
    ...
    parfor i=1:N_sw
        ...
        [filt_temp(:,:,:,i), var_0(i)] = KW_Filter_Adam_20231213(data_temp(:,:,:,i)); 
    end
    filt_temp = reshape(filt_temp, [Win_L, Win_L, T_win, Tot_step_x, Tot_step_y]);
    for i=1:Tot_step_x
        for j=1:Tot_step_y
            for k = 1:N_fr
                x_ind = bp_x(:, k, i, j);
