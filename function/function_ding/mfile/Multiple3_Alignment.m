...
% Multiple3_Alignment, only register to 3 targets to save time

function [b_aligned, bp_x, bp_y] = Multiple3_Alignment(b_0, p_x, p_y) 
...
s_0 = size(b_0);
bp_x = zeros( L_x, s_0(3), 3);
bp_y = zeros( L_y, s_0(3), 3);
b_aligned = zeros(length(p_x), length(p_y), s_0(3), 3);
...
% find the distance to mean patch and then find three targets
mean_patch =  mean(b_0(p_x, p_y, :), 3); % each individual patch, no mean anymore 20231227
mean_patch = (mean_patch - mean(mean_patch(:)))/std(mean_patch(:));

for i=1:s_0(3)
    temp = b_0(p_x, p_y, i);
    temp = (temp - mean(temp(:)))/std(temp(:));
    C_0(i) = abs(sum(temp(:).*mean_patch(:)));
end
[B, I] = sort(C_0, 'descend');
sl_tar = I([1, round(s_0(3)*0.35), round(s_0(3)*0.75)]);

for sl = 1:3 % loop slice
    mean_patch =  b_0(p_x, p_y, sl_tar(sl)); % each individual patch, no mean anymore 20231227
    mean_patch = (mean_patch - mean(mean_patch(:)))/std(mean_patch(:));
