% Read all dicom files from the dicom folder
%clear all,
now_path = pwd; % default path
pname = uigetdir('*.*','Select one folder that you want to read');
...
for N=3:length(all_names) % loop all folders and file names
    if all_names(N).isdir == 1 % This is a directory !
        ...
        if l_names > 1
            ...
            t0 = dicomread( sprintf('%s\\%s\\%s',pname,all_names(N).name,fnames(n_j(1)).name) );
        end
        %t0 = dicomread( fnames(n_j(2)).name );
        s = size(t0);
        a_0 = zeros(s(1),s(2),l_names,'single'); % initialize the original data matrix
...
%         sl(s_Order);
        ...
        filename = all_names(N).name ; save([sprintf('%s\\%s', now_path, filename), '.mat'])
