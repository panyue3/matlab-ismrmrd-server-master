...
L0 = find(v==max(v)); % Find the peak position
a0 = v(1:L0(1));  % This is the array from 1 to first maximum.
L_min_0 = find(a0==min(a0)); % Find the position of minimum in a0.

L_m = find(abs( a0-0.5*v_m(1) ) == min(abs( a0-0.5*v_m(1) ))); % Only search wave front, start from finding center of wavefront.

for i = L_m-1:L0(1); % Search start from wavefront center.
    if 0.5*(v(i+1)+v(i+2))<v(i), L=i;break, end % Find the first local maximum.
end
...
for i= L_min_0(length(L_min_0))+1:L-1 % From minimum position search to maximum position, find the maximum difference.
    if a0(i+1)-a0(i)>a0(i) - a0(i-1)  % To find the maximum jump in the wave front
        m = i;
    end
end
