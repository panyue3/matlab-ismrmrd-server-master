...
%        p(r) = sqrt(2/pi/sig^2)*exp(-((r-u)^2)/(2*sig^2))
...
%
%    p(x;u,sig^2) = sqrt(2/pi/sig^2)*exp(-((x-u).^2)/(2*sig^2))
%
...
%    f(Xn,a)   = sqrt(2/pi/sig^2)*exp(-((Xn-u).^2)/(2*sig^2))
...
y       = sqrt(2/pi/sig2)*exp(-((x_c-u).^2)/(2*sig2));
