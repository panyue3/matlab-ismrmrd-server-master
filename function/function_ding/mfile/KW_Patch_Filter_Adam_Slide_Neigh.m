% 20240327 Modification: add the 4th dimension to deal with 2x interpolation
% 20240611 Modification: add the noise variance parameter deal with temporal interpolation

function [a_f, Noise_map] = KW_Patch_Filter_Adam_Slide_Neigh(a_0, varargin)
...
N_fe = s_0(1);
N_pe = s_0(2);
if (length(s_0) ==4)&&(s_4 > 1)
    disp('Input data has been x2 interpolated ...')
elseif length(s_0) ~=3
...
if nargin == 2  % If there are two input variables
    ...
    if isfield(option, 'noise_var')
        is_n_var_provided = 1;
        N_Variance = option.noise_var(1:step_x:s_0(1), 1:step_y:s_0(2));
    end
end
...
for k= 1:T_step:s_0(3)
    ...
    data_temp = zeros(Win_L, Win_L, T_win, s_4, Tot_step_x, Tot_step_y);
    filt_temp = zeros(Win_L, Win_L, T_win, s_4, N_sw);
    bp_x = zeros(Win_L, N_fr, Tot_step_x, Tot_step_y);
    bp_y = zeros(Win_L, N_fr, Tot_step_x, Tot_step_y);
    var_0 = zeros(1, N_sw);    
    ...
    data_temp = reshape(data_temp, [Win_L, Win_L, T_win, s_4, N_sw]);
    
    % Initialize option_patch as an array of structures
    if ~exist('option_patch', 'var')
        option_patch(N_sw) = struct('WaveletName', []);
        for i=1:N_sw
            option_patch(i).WaveletName = wname;
        end
        if is_n_var_provided   % with provided noise variance
            patch_n_var = reshape(N_Variance, [N_sw, 1] );
            for i=1:N_sw
                option_patch(i).noise_var = patch_n_var(i);
            end
        end
    end
    ...
    parfor i=1:N_sw
        ...
        [filt_temp(:,:,:,:,i), var_0(i)] = KW_Filter_Adam_Neigh(data_temp(:,:,:,:,i), option_patch(i)); 
    end
    filt_temp = reshape(filt_temp, [Win_L, Win_L, T_win, s_4, Tot_step_x, Tot_step_y]);
    for i=1:Tot_step_x
        for j=1:Tot_step_y
            for k = 1:N_fr
                x_ind = bp_x(:, k, i, j);
                y_ind = bp_y(:, k, i, j);
                t_filtered(x_ind, y_ind, k,:) = t_filtered(x_ind, y_ind, k,:) + filt_temp(:, :, k, :, i, j) ;
    ...
    for i_4 = 1:s_4
        a_f(:,:,t_ind,i_4) = a_f(:,:,t_ind,i_4) + t_filtered(:,:,:,i_4)./a_weight;
    end
