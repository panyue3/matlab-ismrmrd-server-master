% function [p_r,qp_diff,m_qp] = find_ang_corr(a2,a2f,p2,p2f,a4,a4f,a5,a5f,p5,p5f,k0)
% p_r is the power as a function of theta, 1:360 curve for each condition.
% m_qp is the maximum qp measure
% embed into a 512 domain.
function [p_r,qp_diff,qp_diff_e_o,m_qp] = find_qp_corr_a2_a4_best_512(a2,a2f,p2,p2f,a4,a4f,a5,a5f,p5,p5f,k0)
...
for A4=a4:100:a4f
    ...
    for A5_0 = a5:10:a5f
        ...
        for i=1:i00(3)
            ...
            in = find(qp==max(qp));
            a00 = ones(512,512)*mean(mean(mean( a0(:,:,in(1)) )));
            a00(101:228,101:228) = a0(:,:,in(1));
            a_fft(1:512,1:512) = abs(fftshift(fft2(a00))) ;
            a = xy2rt(a_fft,257,257,1:200,pi/360:pi/180:2*pi) ;
            e = sum(a(:,4*k0-8:4*k0+8),2); e0 = e;   %plot(e), pause
            temp = max(qp);
            m_qp(a2,n4) = temp(1);
            p_r(a2,n4,1:360) = e(1:360) ;     
            ...
            qp_diff(a2,n4) = mean(abs(a_p-30)); 
            qp_diff_e_o(a2,n4) = abs(sum( a_p(1:2:12)-a_p(2:2:12) ));% pause,
            %qp_diff_corr(a2,n4) = 
    end
end ;   end ;   end ;   end
