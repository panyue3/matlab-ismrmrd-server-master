...
for N=3:length(all_names) % loop all folders and file names
    if all_names(N).isdir == 1 % This is a directory !
        ...
        k = 1 ;
        s_order = 0;
        t_info = dicominfo( sprintf( '%s\\%s\\%s',pname,all_names(N).name,fnames(n_j(1)).name ) );
        if isfield(t_info,'TriggerTime'), ss = 1; else, ss = 0; end % if TriggTime exist, use it to sort image order
        for i=1:l_names
            t_info = dicominfo( sprintf( '%s\\%s\\%s',pname,all_names(N).name,fnames(n_j(i)).name ) );
            ...
            ss = 0; % ding 2009-05-07 for today's data
            if ss == 1 & isfield(t_info, 'TriggerTime'); % If TriggerTime exist, use TriggerTime to sort
                Ordinal_N(i) = t_info.TriggerTime;
            else % If TriggerTime does not exist, use InstanceNumber to sort
                if ischar(t_info.InstanceNumber)
                    Ordinal_N(i) = str2num(t_info.InstanceNumber);
                else
                    Ordinal_N(i) = t_info.InstanceNumber;
                end
            end
            waitbar(i/l_names);
        end
        ...
        Ordinal_N = Ordinal_N( s_Order ); % Sort Ordinal_N
...
%         sl(s_Order);
        ...
        %s = size(a_0);
        clear temp; clear h, clear t0; clear s*, clear fname; clear l_name; clear i, clear j; clear n_j; clear fnames, clear Instance_Number, clear Trigger_Time
        ...
        clear t_info, clear a_*, clear Ordinal_N,
    end
end
