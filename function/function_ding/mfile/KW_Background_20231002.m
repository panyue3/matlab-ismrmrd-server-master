%function [ ImageFiltered, Variance, Threshold, p_value, Cutoff ] = KW_Background_20231002( ImageSeries, varargin )
function [ ImageFiltered, Img_t, b_2 ] = KW_Background_20231002( ImageSeries, varargin )
...
b_1 = ImageSeries(:,:,2:N_training);
b_2 = ImageSeries(:,:,1+N_training:end);
%s_0 = size(a_0);
s_1 = size(b_1);
s_2 = size(b_2);
s = s_2;
clean_a_1_eigen_multip1=zeros(s(1),s(2),s(3));
[a_I, V, D] = KL_Eigenimage(b_1);
% figure(1), semilogy(diag(D))

temp = reshape(a_I, [s_1(1)*s_1(2), s_1(3)]);
temp = sum(abs(temp'*temp)).^0.5;
Eigen_I = reshape(a_I, [s_1(1)*s_1(2), s_1(3)]);
for i=1:s_1(3)
    Eigen_I(:,i) = Eigen_I(:,i)/temp(i);
end
Eigen_I = Eigen_I(:, 9:end);
temp = reshape(b_2, [s_2(1)*s_2(2), s_2(3)]);
temp = Eigen_I'*temp;

Img_t = reshape(Eigen_I*temp, [s_2(1), s_2(2), s_2(3)]);
Img_d = b_2 - Img_t;
[a_I, V, D] = KL_Eigenimage(Img_d);
% figure(2), semilogy(diag(D))

% for j=1:10
% for i=1:s_2(3)
%     figure(1), imagesc([Img_t(:,:,i), b_2(:,:,i), abs(Img_t(:,:,i)-b_2(:,:,i))]), axis image, title(num2str(i)), pause(0.1)
% end
% end

if 3 == 2  % If there are two input variables
%if nargin == 2  % If there are two input variables
    option = varargin{1};
...
%     clean_a_1_eigen_multip1(:,:,m) = iswt2(swa(:,:,end),swh1,swv1,swd1,wname);
    if isreal(a_I)
        ...
        [clean_temp1, Threshold1] = Wavelet_Eig_Filt(real(a_I), m, wname, Variance/2, beta, multip);
        [clean_temp2, Threshold2] = Wavelet_Eig_Filt(imag(a_I), m, wname, Variance/2, beta, multip);
