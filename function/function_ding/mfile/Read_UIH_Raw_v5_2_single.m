...
function [rawdata,phasecor,feed_back,noise_scan,rotAngleInPreScan,rotAngle] = Read_UIH_Raw_v5_2_single(filename) 
rotAngleInPreScan = 0; % Ding 2019-07-24
rotAngle = 0;
...
rawdata=zeros(rawdata_size, 'single');
...
%                 Index(8),Index(9),Index(10),Index(11),Index(12),Index(13))...
%                 =LineData(:,:);
        phasecor(:,:,phaseCorrIndex) = LineData(:,:);
%         rotAngleInPreScan(phaseCorrIndex) = DHL.m_IripPara; % Ding 2019-07-24         
        phaseCorrIndex = phaseCorrIndex + 1;
    else    
        if(DataFormat == 1)
            LineData = ReadLineDataV1(fid,iSamples,iChannels);
        elseif(DataFormat == 2)
        ...
        =single(LineData(:,:));
%         rotAngle(DHL.m_ushSlice+1,DHL.m_ushSPELine+1,DHL.m_ushPELine+1) = DHL.m_IripPara;
    end
...
rawdata = squeeze(single(rawdata));
size(rawdata)
