...
% 20240327 modification: add 4th dimension to deal with interpolation 
% 20240327 modification: Only use the first image series to do alignment
...
b_aligned = zeros(length(p_x), length(p_y), s_0(3), s_4);

p_x = mod(p_x - 1, s_0(1)) + 1;
p_y = mod(p_y - 1, s_0(2)) + 1;

mean_patch = mean( abs(b_0(p_x, p_y, :, 1)), 3 );
...
b_s = b_0(s_x, s_y, :, 1);
s_b = size(b_s);
b_tot = reshape(abs(b_s), [s_b(1), s_b(2)*s_b(3)]);
...
for i=1:s_b(3)
    ...
    a_temp = (b_0(:, :, i, :));
    b_aligned(:,:,i,:) = a_temp(bp_x(:, i), bp_y(:, i), 1, :);
end
